...
> **当前为 Alpha。** Photo Steward 已经可以作为技术用户自行部署的本地服务使用，源代码采用 [Apache-2.0 许可证](./LICENSE)并公开发布。但 macOS App 目前只有完成签名的通用架构验证包，尚未发布经过 Apple 公证的正式安装包。
...
在对方能够自行填写私有配置、理解当前是开发安装的前提下，这个 Alpha 源码可以提供给可信赖的技术用户评估。App 的正式公开分发仍需要完成：
...
源码公开和 App 分发是两个不同阶段。当前源码已经采用 Apache-2.0 并满足公开仓库的隐私检查；但依赖 checkout 软链接的安装方式仍是 Alpha 开发路径，未经公证的 App 也不能视为面向公众的正式发行版。文档中出现 iCloud 和 Photos 只是为了说明支持的 Apple 平台能力；Photo Steward 是独立软件，与 Apple 无隶属关系。
